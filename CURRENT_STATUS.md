...
## 🚀 **Fix V: パーフェクトオーダーに勢いフィルター追加（2026年2月5日）**

### 追加した条件

パーフェクトオーダースクリーニングに、**勢いのある銘柄**を検出するための2つのフィルターを追加しました。
...
rv_ratio = current_volume / avg_volume_1month
if rv_ratio < 3.0:
    return None  # 除外
```
...
**パーフェクトオーダースクリーニングの全条件:**

1. ✅ **パーフェクトオーダー**: 株価 ≥ 10EMA ≥ 20EMA ≥ 50EMA
2. ✅ **乖離率**: (株価 - 50EMA) / 株価 ≤ 20%
3. ✅ **価格上昇率**: 1か月で10%以上上昇 ← **NEW**
4. ✅ **相対出来高**: 1か月平均の3倍以上 ← **NEW**
5. ✅ **データ量**: 50日分以上のデータ
...
```python
{
    ...
    "market": "プライム",
    "volume": 15000000,
    "price_increase_pct": 12.5,  # ← NEW: 1か月の上昇率
    "rv_ratio": 4.2  # ← NEW: 相対出来高倍率
}
```
...
**変更ファイル:**
- `daily_data_collection.py` (Line 656-772)
  - 統計情報に `passed_price_increase` と `passed_volume_increase` 追加
  - 1か月前の価格取得ロジック追加
  - 価格上昇率10%フィルター追加
  - 相対出来高3倍フィルター追加
