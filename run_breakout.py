    ...
    try:
        ...
        try:
            ...
            if prev_result.data:
                ...
                new_stocks = [s for s in breakout_sampled if str(s['code']) not in prev_codes]
                cont_stocks = [s for s in breakout_sampled if str(s['code']) in prev_codes]
                
                logger.info(f"📊 新規検出: {len(new_stocks)}銘柄 / 継続: {len(cont_stocks)}銘柄")
                
                MAX_TOTAL = 90
                if len(new_stocks) >= MAX_TOTAL:
                    breakout_sampled = new_stocks[:MAX_TOTAL]
                else:
                    remaining = MAX_TOTAL - len(new_stocks)
                    import random
                    random.shuffle(cont_stocks)
                    breakout_sampled = new_stocks + cont_stocks[:remaining]
                
                logger.info(f"📊 最終保存: {len(breakout_sampled)}銘柄（新規優先）")
